...
class opinion_dynamics:
    ...
    def __init__(
        ...
        stochiomatric_vectors: np.ndarray,
        grad_V: str = "none",
        grad_V_params: tuple[float, ...] = (0, 0),
        ...
        self.opinion_array = np.zeros(
            (num_grid_points, len(initial_opinions)), dtype=float
        )
        ...
        self.agent_state = np.zeros(
            len(initial_opinions), dtype=int
        )  # (0, 1, 2) for (S, I, R)
        inital_inf_idx = np.random.choice(
            len(initial_opinions), size=self.infection_curr[1], replace=False
        )
    ...
    def algo(self):
        for i in range(1, len(self.t)):
            ...
            if self.grad_V == "sigmoid":
                ...
                self.opinions_curr -= (
                    4 * self.opinions_curr * (self.opinions_curr**2 + 1)
                    - 8
    ...
    def infection_propensity(self):
        bin_idx = np.clip(
            (self.opinions_curr - HIST_BINS[0]).astype(int), 0, len(HIST_BINS) - 2
        )
        bin_susceptible_counts = np.bincount(
            bin_idx[self.agent_state == 0], minlength=len(HIST_BINS) - 1
        )
        self.bin_counts = np.bincount(bin_idx, minlength=len(HIST_BINS) - 1)
        bin_sum = np.bincount(
            bin_idx, weights=self.opinions_curr, minlength=len(HIST_BINS) - 1
        )
        bin_mean = bin_sum / np.maximum(self.bin_counts, 1)
        propensities = (
            self.inf_rate_max_plus_min
            - self.inf_rate_max_minus_min * np.tanh(2 * bin_mean)
        ) / 2
        return (
            bin_idx,
            propensities * bin_susceptible_counts * self.infection_curr[1] / self.N,
        )
    ...
    def update_infected(self) -> float:
        ...
        for bin_i in range(len(infected_bins)):
            if infected_bins[bin_i] > 0:
                ...
                candidates = np.flatnonzero(
                    (self.agent_state == 0) & (bin_idx == bin_i)
                )
                chosen = np.random.choice(
                    candidates,
                    size=min(infected_bins[bin_i], len(candidates)),
                    replace=False,
                )
        ...
        chosen = np.random.choice(
            candidates, size=min(rec_jumps, len(candidates)), replace=False
        )
